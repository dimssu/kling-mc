...
export default function VideoFromImagesDetailPage({ params }: Props) {
  const { id } = use(params);
  const q = useQuery({
    queryKey: ["video-generation", id],
    queryFn: () => api.getVideoGeneration(id),
    refetchInterval: ({ state }) => {
      const status = state.data?.videoGeneration.status;
      return status === "queued" || status === "processing" ? 4_000 : false;
    },
  });

  const g = q.data?.videoGeneration;
      ...
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <Card>
          <CardContent className="py-10 text-center">
            <p className="font-medium">Video generation not found</p>
          </CardContent>
        </Card>
      </div>
    );
