...
import { api, type VideoGeneration } from "@/lib/api-client";
import { formatUsd, relativeTime } from "@/lib/utils";
import { StatusPill } from "@/components/StatusPill";
import { MediaThumb } from "@/components/MediaThumb";
import { Button } from "@/components/ui/button";

export function VideoGenerationCard({ gen }: { gen: VideoGeneration }) {
  const qc = useQueryClient();

  const fav = useMutation({
    mutationFn: () => api.toggleVideoGenerationFavorite(gen.id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["video-generations"] }),
  });

  const del = useMutation({
    mutationFn: () => api.deleteVideoGeneration(gen.id),
    onSuccess: () => {
  ...
  const firstRef = gen.referenceImages?.find((r) => r) ?? null;
  const display = gen.outputAsset ?? firstRef ?? null;
  const endpointLabel =
    gen.endpoint === "image2video" ? "Image → video" : "Multi-image → video";
  ...
  return (
    <div className="group relative flex flex-col gap-3 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-elev)] p-3 transition-colors hover:border-[var(--color-border-strong)]">
      <Link href={`/library/video-from-images/${gen.id}`} className="contents">
        {display ? (
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 space-y-1">
          <Link
            href={`/library/video-from-images/${gen.id}`}
      ...
      <div className="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <Button
          ...
          aria-label="Delete"
          onClick={() => {
            if (window.confirm("Delete this generation? This also removes the generated video.")) {
              del.mutate();
            }
          }}
        >
